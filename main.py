...
# ════════════════════════════════════════════════════════════
    ...
    if run_btn:
        ...
        if sim_type == "🎲 주사위":
            ...
            fig_bar.add_hline(
                y=int(trials) / dice_faces,
                line_dash="dash", line_color="red",
                annotation_text=f"이론값 ({int(trials)//dice_faces}회)",
        ...
        # ── 동전 ──────────────────────────────────────────
            ...
            c1, c2 = st.columns(2)
            c1.metric("앞면 (HEAD)", f"{heads}회", f"{round(heads/int(trials)*100,1)}%")
            c2.metric("뒷면 (TAIL)", f"{tails}회", f"{round(tails/int(trials)*100,1)}%")
            ...
            df = pd.DataFrame({
                "결과": ["앞면", "뒷면"],
                "출현 횟수": [heads, tails],
                "실제 확률 (%)": [round(heads/int(trials)*100, 2), round(tails/int(trials)*100, 2)],
                "이론 확률 (%)": [50.0, 50.0],
                "차이 (%)": [round(heads/int(trials)*100 - 50, 2), round(tails/int(trials)*100 - 50, 2)]
            })
            st.dataframe(df, use_container_width=True, hide_index=True)
